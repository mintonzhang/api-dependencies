...
import cn.minsin.wechat.miniprogram.model.WxPhoneNumberModel;
...
import cn.minsin.wechat.wechatpay.core.util.WxDecryptUtil;
...
public class WechatMiniProgramFunctions extends WeChatPayFunctions {
	 ...
	 */
	public Code2SessionReturnModel getSessionKey(String code) throws Exception {
	 ...
	 * @param iv            加密算法的初始向量
	 */
	public UserInfoModel getUser(String encryptedData, String sessionKey, String iv)
			throws Exception {
		// 被加密的数据
	 ...
	 */
	public Map<String, String> getPay(MiniProgramOrderPayModel model)
			throws Exception {
	 ...
	 */
	public RefundReturnModel getRefund(MiniProgramRefundModel model)
			throws Exception {
		model.setAppid(properties.getAppid());
	 ...
	 * 解密手机号
	 */
	public WxPhoneNumberModel getPhone(String sessionKey, String encryptedData, String iv) throws Exception {
		return WxPhoneNumberModel.fromJson(WxDecryptUtil.decrypt(encryptedData, sessionKey, iv));
	}
	...
	public AccessTokenModel getAccessToken() throws Exception {
		...
		try {
			...
			String string = EntityUtils.toString(response.getEntity(), StandardCharsets.UTF_8.toString());
	 ...
	 */
	public InputStream getQRCode(MiniProgramCodeModel model)
			throws Exception {
		...
		try {
			...
			entity.setContentEncoding(new BasicHeader(HTTP.CONTENT_TYPE, StandardCharsets.UTF_8.toString()));
			httpPost.setEntity(entity);
			response = httpClient.execute(httpPost);
			InputStream content = response.getEntity().getContent();
			byte[] copyInputStream = IOUtil.readAllBytes(content);
